document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- OVERVIEW STATS & RECENT ORDERS RENDERER ---
    function renderOverviewStats() {
        ...
        // Render Recent Orders dynamically under Overview
        let recentOrdersContainer = overviewSec.querySelector('.recent-orders-container');
        if (!recentOrdersContainer) {
            recentOrdersContainer = document.createElement('div');
            recentOrdersContainer.className = 'recent-orders-container';
            overviewSec.appendChild(recentOrdersContainer);
        }

        if (orders.length === 0) {
            recentOrdersContainer.innerHTML = `
                <h3 class="section-title">Recent orders</h3>
                <p style="color:var(--text-muted);">No recent orders recorded.</p>
            `;
            return;
        }

        const recentOrdersHtml = orders.slice(-5).reverse().map(o => `
